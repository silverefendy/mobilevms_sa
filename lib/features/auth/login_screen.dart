...
const _kLastCompanyCode = 'vms_last_company_code';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _loadSavedCredentials() async {
    try {
      ...
      final savedUsername = prefs.getString(_kLastUsername);
      final savedCompanyCode = prefs.getString(_kLastCompanyCode);
      if (savedUsername != null && savedUsername.isNotEmpty && mounted) {
        _usernameController.text = savedUsername;
      }
      if (savedCompanyCode != null && savedCompanyCode.isNotEmpty && mounted) {
  ...
  Future<void> _saveCredentials(String username, String companyCode) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(_kLastUsername, username.trim());
      await prefs.setString(_kLastCompanyCode, companyCode.trim());
    } catch (_) {}
  }
  ...
  Future<void> _submit() async {
    ...
    if (!mounted) return;
    if (ok) {
      await _saveCredentials(username, companyCode);
  ...
  String _friendlyError(String? raw) {
    ...
    if (lower.contains('incorrect') || lower.contains('invalid') ||
      ...
      return 'Username, password, atau company code salah. Periksa kembali.';
    ...
    if (lower.contains('not found'))
      return 'Company tidak ditemukan. Periksa company code.';
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      autocorrect: false,
                      onFieldSubmitted: (_) => _companyCodeFocus.requestFocus(),
                      validator: (v) =>
                    ...
                    TextFormField(
                      ...
                      validator: (v) =>
                          (v == null || v.trim().isEmpty)
                              ? 'Company code tidak boleh kosong'
                              : null,
                    ),
                    const SizedBox(height: 14),
                    TextFormField(
                    ...
                    const SizedBox(height: 20),
                    Center(
                      child: Text('Connected to VMS Backend',
                          style: TextStyle(
                              fontSize: 11,
